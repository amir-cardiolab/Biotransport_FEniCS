...
relative_tolerance = 1e-15 #1e-6 This was original value
...
results_dir = root_dir + 'ATP_transport/Results/'
...
#mesh = Mesh()
#hdf = HDF5File(mpi_comm_world(), root_dir + "mesh_transport/" + "1A_2012_LCx.h5", "r")
...
#********************************************************
#*************** ATP Transport Parameters ***************
#********************************************************

D = 2.36e-6 #cm2/s : Diffusivity
D_high = 1e-5
K_m = 475e-9 #mol/cm3 : Michaelis Constant for the Enzyme
V_max = 0.8e-10 #mol/cm2.s : Maximum Enzyme Reaction VElocity for ATP Hydrolysis
S_max = 10e-13 #mol/cm2.s : Maximum Release Rate of ATP
tau_0 = 1 #pa=g/cm.s2 (Intermediate Sigmoidal ATP Release) : Reference Shear Stress Regulating the Rate at which the Maximum ATP Release is Attained. =0.01 Rapid, =1 Slow
c_0 = 0.1e-9 #mol/cm3 : ATP inlet concentration

c_0_space = interpolate(Constant(c_0), Q)
S_max_space = interpolate(Constant(S_max), Q)
...
c = Function(Q, name="ATP Concentration")
...
file_conc_output = File(results_dir + 'ATP_Concentration.pvd')
while tstep < n_tsteps:
	...
	for tstep_inter in xrange(tsteps_per_file):
		...
		exp_arg = (-1)*mag_wss/tau_0
		exp_term = ufl_op.exp(exp_arg)
		S_ATP = S_max*pow(1 - exp_term,3)



		n = - v * dot(velocity , grad(c_trial) ) * dx \
			- D_multiple * dot(grad(v), grad(c_trial)) * dx \
			- (V_max / (K_m + c_prev))*c_trial* v * ds(2)
		...
		N = assemble(n)
		N_rhs = assemble(rhs)  #added by amir. this is vector N is matrix 
