...
while t < (T - tstep * DOLFIN_EPS) and not stop:
    ...
    while udiff[0] > max_error and inner_iter < num_iter:
        ...
        if (Resistance_flag):
         ...
         Res_vec = [Res1, Res2, Res3, Res4, Res5, Res6, Res7, Res8, Res9, Res10, Res11, Res12, Res13, Res14, Res15, Res16, Res17, Res18, Res19, Res20, Res21, Res22, Res23, Res24, Res25, Res26, Res27, Res28, Res29, Res30, Res31]

         if tstep == 1:
            
            Res_vec_tmp = numpy.ones(31)

         if (tstep>1 and tstep<200):
            Res_vec_tmp = tstep*numpy.asarray(Res_vec)/200.0


         if (tstep > 200 and tstep%10==0):
           ...
           for i in xrange(31):
              ...
              if (current_ratios[i] < 0.99*target_ratios[i] or current_ratios[i] > 1.01*target_ratios[i]):
                  Res_vec_tmp[i] *= current_ratios[i]/target_ratios[i]
                  if MPI.rank(mpi_comm_world()) == 0:
                      print 'Res',i,'has changed!'
                      print 'ratio(',i,') = ', current_ratios[i]/target_ratios[i]
