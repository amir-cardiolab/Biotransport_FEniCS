...
bc_file =  '/scratch/mm4238/prostate_CFD/prostate_ext/mesh/BCnodeFacets.xml'
...
while t < (T - tstep * DOLFIN_EPS) and not stop:
    ...
    while udiff[0] > max_error and inner_iter < num_iter:
        ...
        if (Resistance_flag and tstep > 49):
         ...
         outflow_rates = inflow * numpy.asarray(target_ratios)
         Res_vec_tmp = numpy.divide(13332.24*numpy.ones(31), outflow_rates)
         delta_Q = numpy.ones(31)

         # if tstep == 1:
            
         #    Res_vec_tmp = numpy.divide(Res_vec,Res_vec)

         # if (tstep>1 and tstep<200):
         #    Res_vec_tmp = tstep*Res_vec/200


         if (tstep > 49 and tstep%10==0):
         # if (1):
           ...
           current_ratios = flow_vec/(inflow + 1e-16)
           ...
           for i in xrange(31):
              # Res_val = Res_vec[i]
              # delta_Q[i] = outflow_rates[i] - flow_vec[i]
              delta_Q[i] =  flow_vec[i] - outflow_rates[i]
              if (delta_Q[i] > 1e-03 or delta_Q[i] < -1e-03):
                  Res_vec_tmp[i] -= delta_Q[i]/13332.24
                  if (Res_vec_tmp[i] < 0):
                      Res_vec_tmp[i] = 1.0
                  if MPI.rank(mpi_comm_world()) == 0:
                      print 'Res',i,'has changed!'
                      print 'ratio(',i,') = ', flow_vec[i]/outflow_rates[i]
                      print 'Delta_Q(',i,') = ', delta_Q[i]
           ...
           #      ratios_threshold[i] = ratios[i] < 0.995 or ratios[i] > 1.005
