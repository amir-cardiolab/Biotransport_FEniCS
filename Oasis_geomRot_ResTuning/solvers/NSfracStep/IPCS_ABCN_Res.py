...
def pressure_assemble(Resistance_flag,Res1, Res2, Res3,Q,n_normal,facet_domains,mesh,bcs,assemble_matrix, u_,b, x_, dt, Ap,Ap2, divu, **NS_namespace):
...
def pressure_solve(Resistance_flag,Res1, Res2, Res3,Q,n_normal,facet_domains,mesh,assemble_matrix,u_,dp_, x_, Ap,Ap2, b, p_sol, bcs, **NS_namespace):
    ...
    if (Resistance_flag):
       ...
       total_flow = flow1 + flow2 + flow3
       ratios = numpy.zeros(3)
       current_ratios = numpy.zeros(3)
       ratios_threshold = numpy.zeros(3) 
       target_ratios = [0.3636, 0.54, 0.097]
