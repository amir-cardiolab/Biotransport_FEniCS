...
def setup(RT_time,RT_thresh,u_components,backflow_flag,nonNewtonian_flag, beta_backflow,Resistance_flag,n_normal,facet_domains, u, v, p, q, bcs, les_model, nu, nut_,
    ...
    if(nonNewtonian_flag and backflow_flag):
        ...
        #Carreau-Yasuda model #!!!! teeeest
        #visc_nu_0 = (1./1.06) * (0.0345 + (1.56 - 0.0345) * (1. + (1.902*visc_gamma)**1.25 )**( (0.22-1.)/1.25 )  )
        visc_nu_0 = (1./1.06) * (0.345 + (4.56 - 0.345) * (1. + (1.902*visc_gamma)**1.25 )**( (0.22-1.)/1.25 )  )
        visc_nu =   RT_time * visc_nu_0 + (1.0 - RT_time ) * nu
        ...
        K = lhs( visc_nu*inner(grad(u), grad(v)) * dx - 2.0*beta_backflow *  inner(v, ( dot(u_,n_normal) - abs( dot(u_,n_normal) ) )/2. * u ) * (ds(3, domain=mesh, subdomain_data=facet_domains) + ds(4, domain=mesh, subdomain_data=facet_domains) )   )
         ...
         visc_nu_0 = (1./1.06) * (0.0345 + (0.56 - 0.0345) * (1. + (1.902*visc_gamma)**1.25 )**( (0.22-1.)/1.25 )  )
         visc_nu =  step_fn(RT_time,RT_thresh) * visc_nu_0 + (1.0 -  step_fn(RT_time,RT_thresh) ) * nu
         #visc_gamma = pow(0.5*Strain(u_ab_prev), 0.5)
