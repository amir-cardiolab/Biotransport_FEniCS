...
def setup(u_components,backflow_flag,nonNewtonian_flag, beta_backflow,Resistance_flag,n_normal,facet_domains, u, v, p, q, bcs, les_model, nu, nut_,
    ...
    if(nonNewtonian_flag and backflow_flag):
	 ...
	 ds = Measure("ds")(subdomain_data=facet_domains)
         #ds = Measure("ds")[facet_domains]
         K = lhs( nu*inner(grad(u), grad(v)) * dx - 2.0*beta_backflow *  inner(v, ( dot(u_,n_normal) - abs( dot(u_,n_normal) ) )/2. * u ) * (ds(3, domain=mesh, subdomain_data=facet_domains) + ds(4, domain=mesh, subdomain_data=facet_domains) + ds(5, domain=mesh, subdomain_data=facet_domains) + ds(6, domain=mesh, subdomain_data=facet_domains) )   )
         visc_nu = nu
    else:
         K = assemble_matrix(inner(grad(u), grad(v)) * dx)
         visc_nu = nu
...
def pressure_assemble(Resistance_flag,Res1, Res2,Res3, Res4, Q,n_normal,facet_domains,mesh,bcs,assemble_matrix, u_,b, x_, dt, Ap,Ap2, divu, **NS_namespace):
...
def pressure_solve(Resistance_flag,Res1, Res2, Res3, Res4, Q,n_normal,facet_domains,mesh,assemble_matrix,u_,dp_, x_, Ap,Ap2, b, p_sol, bcs, **NS_namespace):
    ...
    if (Resistance_flag):
       ...
       if MPI.rank(mpi_comm_world()) == 0:
        ...
        print '+++++++++++++++++++++++Res_bc: out_1 to inlet ratio:', flow1/(inflow + 1e-16)
        print '+++++++++++++++++++++++Res_bc: out_2 to inlet ratio:', flow2/(inflow + 1e-16)
        print '+++++++++++++++++++++++Res_bc: out_3 to inlet ratio:', flow3/(inflow + 1e-16)
        print '+++++++++++++++++++++++Res_bc: out_4 to inlet ratio:', flow4/(inflow + 1e-16)
        print '+++++++++++++++++++++++Res_bc: total outflow:', flow1 + flow2 + flow3 + flow4
        print '+++++++++++++++++++++++     inlet_bc: inflow:', inflow
       bcs['p'] = [DirichletBC(Q,  Res_bc1 , facet_domains,3), DirichletBC(Q,  Res_bc2, facet_domains,4), DirichletBC(Q,  Res_bc3, facet_domains,5), DirichletBC(Q,  Res_bc4, facet_domains,6)]
       #Ap = assemble_matrix(Ap2, bcs['p'])
