...
def setup(u_components,backflow_flag,nonNewtonian_flag, beta_backflow,Resistance_flag,n_normal,facet_domains, u, v, p, q, bcs, les_model, nu, nut_,
    ...
    if(nonNewtonian_flag and backflow_flag):
        ...
        K = lhs( visc_nu*inner(grad(u), grad(v)) * dx - 2.0*beta_backflow *  inner(v, ( dot(u_,n_normal) - abs( dot(u_,n_normal) ) )/2. * u ) * (ds(3, domain=mesh, subdomain_data=facet_domains) + ds(4, domain=mesh, subdomain_data=facet_domains) + ds(5, domain=mesh, subdomain_data=facet_domains) )   )
         ...
         K = lhs( nu*inner(grad(u), grad(v)) * dx - 2.0*beta_backflow *  inner(v, ( dot(u_,n_normal) - abs( dot(u_,n_normal) ) )/2. * u ) * (ds(3, domain=mesh, subdomain_data=facet_domains) + ds(4, domain=mesh, subdomain_data=facet_domains) + ds(5, domain=mesh, subdomain_data=facet_domains) + ds(6, domain=mesh, subdomain_data=facet_domains) + ds(7, domain=mesh, subdomain_data=facet_domains)  )   )
         visc_nu = nu
    else:
         K = assemble_matrix(inner(grad(u), grad(v)) * dx)
         visc_nu = nu
...
def pressure_solve(Resistance_flag,Res1, Res2, Res3, Res4, Res5, tstep, Q,n_normal,facet_domains,mesh,assemble_matrix,u_,dp_, x_, Ap,Ap2, b, p_sol, bcs, **NS_namespace):
    """Solve pressure equation."""
    if (Resistance_flag and tstep>20):
     if MPI.rank(mpi_comm_world()) == 0:
         print 'Res1_initial = ', Res1, 'Res2_initial = ', Res2, 'Res3_initial = ', Res3, 'Res4_initial = ', Res4, 'Res5_initial = ', Res5
...
#                print 'Res1 has changed!'
     ...
     if MPI.rank(mpi_comm_world()) == 0:
        ...
        print '+++++++++++++++++++++++ Res_bc: total outflow:', flow1 + flow2 + flow3 + flow4 + flow5
        print '+++++++++++++++++++++++ inlet_bc: inflow:', inflow
        print '+++++++++++++++++++++++ Res1 = ', Res1, 'Res2 = ', Res2, 'Res3 =', Res3, 'Res4 =', Res4, 'Res5 =', Res5
     bcs['p'] = [DirichletBC(Q,  Res_bc1 , facet_domains,3), DirichletBC(Q,  Res_bc2, facet_domains,4), DirichletBC(Q,  Res_bc3, facet_domains,5), DirichletBC(Q,  Res_bc3, facet_domains,6), DirichletBC(Q,  Res_bc3, facet_domains,7)]
       #Ap = assemble_matrix(Ap2, bcs['p'])
...
def IC_Laplacian(V, V_ave, q_, q_1, q_2, sys_comp, u_components, facet_domains, inlet_flag_flow, mesh,
                  Res1, Res2, Res3, Res4, Res5, Resistance_flag, **NS_namespace):
