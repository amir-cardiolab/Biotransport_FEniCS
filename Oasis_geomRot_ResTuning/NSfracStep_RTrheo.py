...
file_stop = 19
file_interval = 1
tsteps_per_file = 105 #(T=1.053/19)/(dt)
...
RT_nparray = numpy.zeros(num_nodes)
...
while t < (T - tstep * DOLFIN_EPS) and not stop:
    ...
    for tstep_inter in xrange(tsteps_per_file):
     ...
     RT_time.vector().set_local((1. - alpha_interp) * RT_initial.vector().get_local()+ alpha_interp *  RT_final.vector().get_local())
     for i in xrange(num_nodes_p):
              RT_nparray[i] = 1.0 * ( RT_time.vector()[i] > RT_thresh) #RT_time.vector()[i] does not work in parallel!
     RT_time.vector().set_local(RT_nparray[:])
     RT_time.vector().apply('')
     
     File('/scratch/aa3878/18_oasis/RT_check.pvd') << RT_time #works!
