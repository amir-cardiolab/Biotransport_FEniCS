...
while t < (T - tstep * DOLFIN_EPS) and not stop:
    ...
    # Added by Mostafa: Tunning resistances at outlets according to goal_flows
    if Resistance_flag:
      for i in xrange(2, 400):
        ...
        if  tstep == checkpoint:
          if MPI.rank(mpi_comm_world()) == 0:
            print 'CHECK POINT!'
          outlet_flows = numpy.zeros(3)
          total_flow = 0.
          ratios = numpy.zeros(3)
          ratios_threshold = numpy.zeros(3) 
          goal_flows = [0.3636, 0.54, 0.097]


          for i in xrange(3,5):
            j = i - 3
            outlet_flows[j] = abs( assemble(dot(u_, n_normal)*ds(i, domain=mesh, subdomain_data=facet_domains) ) ) 
            total_flow += outlet_flows[j]

          for i in xrange(0,2): # if Resistances initiate from 0.0 then need to add an if condition
            ratios[i] = outlet_flows[i] / total_flow / goal_flows[i]
            ratios_threshold[i] = ratios[i] < 0.999 or ratios[i] > 1.001
            if ratios_threshold[i] and i == 0:
              Res1 *= ratios[i]
              if MPI.rank(mpi_comm_world()) == 0:
      ...
      if  tstep % 10 == 0 and MPI.rank(mpi_comm_world()) == 0:
              print 'Resistances: ', Res1, Res2, Res3
