...
dsi = numpy.zeros(25)
A = numpy.zeros(25)
normal_x = numpy.zeros(25)
normal_y = numpy.zeros(25)
normal_z = numpy.zeros(25)
x = SpatialCoordinate(mesh)
ds = Measure("ds")(subdomain_data=facet_domains)
for i in xrange(25):
  dsi = ds(i+3, domain=mesh, subdomain_data=facet_domains)
  ni = numpy.array([assemble(n_normal[j]*dsi) for j in xrange(d)])
  n_len = numpy.sqrt(sum([ni[j]**2 for j in xrange(d)])) # Should always be 1!?
  A[i] = assemble(Constant(1.0, name="one")*dsi)
  normal_x[i] = ni[0]/n_len
  normal_y[i] = ni[1]/n_len
  normal_z[i] = ni[2]/n_len
  if MPI.rank(mpi_comm_world()) == 0:
    print 'Normal vector for outlet #',i+1, ' is: [', normal_x[i], ',', normal_x[i], ',', normal_x[i], ']'
...
while t < (T - tstep * DOLFIN_EPS) and not stop:
    ...
    while udiff[0] > max_error and inner_iter < num_iter:
        ...
        if (tstep > 20 and Resistance_flag):
