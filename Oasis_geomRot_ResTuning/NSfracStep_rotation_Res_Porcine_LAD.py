...
#Rotating the mesh so that the average normal vector aligned with z-axis passing the centroid
      ...
      alpha_ang = abs(acos(normal[0])*180./pi)
      beta_ang = abs(acos(normal[1])*180./pi)
      gamma_ang = abs(acos(normal[2])*180./pi)
      ...
      mesh.rotate(270 + gamma_ang, 1, center)
    ...
    while udiff[0] > max_error and inner_iter < num_iter:
        ...
        if (Resistance_flag):
         ...
         if tstep > 20:
           ...
           total_flow = flow1 + flow2 + flow3 + flow4 + flow5 + flow6 + flow7 + flow8 + flow9
           ratios = numpy.zeros(9)
           current_ratios = numpy.zeros(9)
           ratios_threshold = numpy.zeros(9)
           target_ratios = [0.3590, 0.0809, 0.0714, 0.0814, 0.0966, 0.0603, 0.0487, 0.0530, 0.1487]
