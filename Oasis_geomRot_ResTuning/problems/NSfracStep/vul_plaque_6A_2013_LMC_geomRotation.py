...
import vul_plaque_6A_2013_LMC_SS_BC
...
def mesh(**params):
    ...
    m = Mesh('/scratch/mm4238/oasis_geometries/6A_2013/LMC/mesh/6A_2013_LMC.xml')
    return m
...
if restart_folder:
        ...
        NS_parameters.update(
            solver="IPCS_ABCN_Res_6A_2013_LMC",
            nu=0.04/(1.06),
            T=  1.0*3,
            dt= 1.0 /(4000. * 5.) , # 0.95 /(2000.*5.)
            folder= '/scratch/mm4238/oasis_geometries/6A_2013/LMC/Results',
            velocity_degree=2,
            save_step= 1 * 1, #40 * 5
            save_start= 1, #20000,
            checkpoint=100*1e9,
            ...
            Res6 = 1e-6, #resistance at the third outlet
            Res7 = 1e-6, #resistance at the third outlet
            inlet_rotation = False,
            IC_lap = False, # True if Laplacian initialization is needed
            V_ave = 0.2, # average velocity/flowrate for initialization
...
def create_bcs(t,t_cycle,Time_array,v_IR_array, V, Q, facet_domains, mesh, **NS_namespace):
    ...
    return dict(u0=[bc_in_1, bc_wall_1],
                ...
                p=[bcp1, bcp2, bcp3, bcp4, bcp5, bcp6, bcp7])
    #return dict(u0=[bc_in_1, bc_wall_1],
    #           u1=[bc_in_2, bc_wall_2],
    #           u2=[bc_in_3, bc_wall_3],
    #           p =[]) # no pressure BC
...
def pre_solve_hook(mesh, facet_domains, velocity_degree, u_,
    ...
    Time_array = vul_plaque_6A_2013_LMC_SS_BC.time_BC()
    v_IR_array = vul_plaque_6A_2013_LMC_SS_BC.Vel_IR_BC()
