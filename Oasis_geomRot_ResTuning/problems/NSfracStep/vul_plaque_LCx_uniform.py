...
def mesh(**params):
    ...
    m = Mesh('/scratch/mm4238/utilities/new_BL_3M/1A_2012_LAD_mesh.xml')
    return m
...
if restart_folder:
        ...
        NS_parameters.update(
            ...
            nu=0.004/(1.06/1000),
            T=  1.0*3,
            dt= 1.0 /(2000.) , # 0.95 /(2000.*5.)
            folder= '/scratch/mm4238/utilities/new_BL_3M',
            velocity_degree=1,
            save_step= 10, #40 * 5
            save_start= 10,
            checkpoint=100*1e9,
            ...
            Res1 = 1., #resistance at the first outlet
            Res2 = 1., #resistance at the second outlet
            Res3 = 1., #resistance at the third outlet
            flag_wss = True,
            flag_ramp = False, #for the first cycle only start from 0 flow rate at the inlet
            initial_time_ramp = -0.001, # should be a negative value. Same value entered at the first time entry in the BC file with a corresponding zero flow.
            use_krylov_solvers=True)
        NS_parameters['krylov_solvers']['monitor_convergence'] = True
        #set_log_level(ERROR)
        globals().update(NS_parameters)

# Changed by Mostafa: V_inletBC_x,V_inletBC_y,V_inletBC_z, facet_domains
def create_bcs(t,t_cycle,Time_array,v_IR_array, V, Q, V_inletBC_x,V_inletBC_y,V_inletBC_z, facet_domains, mesh, **NS_namespace):
#def create_bcs(t,t_cycle,Time_array,v_IR_array, V, Q, mesh, **NS_namespace):
    ...
    #bc_file =  '/scratch/mm4238/utilities/new_BL_3M/BCnodeFacets.xml'
    ...
    if (parabolic_flag):
        ...
        #V_inletBC_x = Function(V,'/scratch/mm4238/utilities/new_BL_3M/bctxml_x.xml')
        #V_inletBC_y = Function(V,'/scratch/mm4238/utilities/new_BL_3M/bctxml_y.xml')
        #V_inletBC_z = Function(V,'/scratch/mm4238/utilities/new_BL_3M/bctxml_z.xml')
        V_inlet_in_x = (-1)*V_inletBC_x*(v_inlet_BC)*1000
        V_inlet_in_y = (-1)*V_inletBC_y*(v_inlet_BC)*1000
        V_inlet_in_z = (-1)*V_inletBC_z*(v_inlet_BC)*1000
	...
	bc_in_2  = DirichletBC(V, V_inlet_in_y , facet_domains,inlet_facet_ID) #inlet for u_y
       ...
       n_len = numpy.sqrt(sum([ni[i]**2 for i in xrange(d)])) # Should always be 1!?
       normal = ni/n_len
       bc_u0 = Expression("n0 * u * 1000",n0=normal[0], u=-v_inlet_BC, degree=3)
       bc_u1 = Expression("n0 * u * 1000",n0=normal[1], u=-v_inlet_BC, degree=3)
       bc_u2 = Expression("n0 * u * 1000",n0=normal[2], u=-v_inlet_BC, degree=3)
       bc_in_1  = DirichletBC(V, bc_u0 , facet_domains,inlet_facet_ID) #inlet for u_x
       bc_in_2  = DirichletBC(V, bc_u1 , facet_domains,inlet_facet_ID) #inlet for u_y
       bc_in_3  = DirichletBC(V, bc_u2 , facet_domains,inlet_facet_ID) #inlet for u_z
