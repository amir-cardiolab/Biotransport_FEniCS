...
import porcine_4609_LAD_BC
#import porcine_4609_LAD_SS_BC
...
def mesh(**params):
    ...
    m = Mesh('/scratch/mm4238/transport/porcines/4609_B/mesh/4609_LAD_baseline.xml')
    return m
...
if restart_folder:
        ...
        NS_parameters.update(
            solver="IPCS_ABCN_Res_Porcine_4609_LAD",
            nu=0.04/(1.06),
            T=  0.6044*3,
            dt= 0.6044 /(4000. * 5.) , # 0.95 /(2000.*5.)
            folder= '/scratch/mm4238/transport/porcines/4609_B/Results_IC',
            velocity_degree=2,
            save_step= 40 * 5, #40 * 5
            save_start= 2000,
            checkpoint=100*1e9,
            ...
            Res1 = 170.56, #resistance at the first outlet
            Res2 = 703.28, #resistance at the second outlet
            Res3 = 0.0, #resistance at the third outlet
            Res4 = 1912.74, #resistance at the fourth outlet
            Res5 = 1067.55, #resistance at the fourth outlet
            Res6 = 1213.22, #resistance at the fourth outlet
            Res7 = 1784.56, #resistance at the fourth outlet
            Res8 = 1576.10, #resistance at the fourth outlet
            Res9 = 151.00, #resistance at the fourth outlet
            inlet_rotation = False,
            IC_lap = True, # True if Laplacian initialization is needed
            V_ave = 3.5684, # average velocity/flowrate for initialization
...
def create_bcs(t,t_cycle,Time_array,v_IR_array, V, Q, facet_domains, mesh, **NS_namespace):
    ...
    return dict(u0=[bc_in_1, bc_wall_1],
                ...
                p=[bcp1, bcp2, bcp3, bcp4, bcp5, bcp6, bcp7, bcp8, bcp9])
    #return dict(u0=[bc_in_1, bc_wall_1],
    #           u1=[bc_in_2, bc_wall_2],
    #           u2=[bc_in_3, bc_wall_3],
    #           p =[]) # no pressure BC
...
def pre_solve_hook(mesh, facet_domains, velocity_degree, u_,
    ...
    Time_array = porcine_4609_LAD_BC.time_BC()
    v_IR_array = porcine_4609_LAD_BC.Vel_IR_BC()
