...
bc_file =  '/scratch/mm4238/oasis_geometries/6A_2013/LMC/mesh/BCnodeFacets.xml'
    ...
    while udiff[0] > max_error and inner_iter < num_iter:
        ...
        if (Resistance_flag):
         ...
         if tstep > 20:
           ...
           total_flow = flow1 + flow2 + flow3 + flow4 + flow5 + flow6 + flow7
           ratios = numpy.zeros(7)
           current_ratios = numpy.zeros(7)
           ratios_threshold = numpy.zeros(7)
           target_ratios = [0.16445, 0.36879, 0.06031, 0.09661, 0.08220, 0.02722, 0.20042]
