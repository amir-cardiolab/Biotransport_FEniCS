...
results_dir = root_dir + 'O2_transport/Results/'
...
c_0 = 1.305e-6#1.305e-12 # [mol/mm3]
c_0_space = interpolate(Constant(c_0), Q)

c_wall = 1.044e-6#1.044e-12 # [mol/mm3]
...
c = Function(Q, name="O2 Concentration")
...
flux_sol = Function(Q, name="O2 Flux")
...
bc2 = DirichletBC(Q, c_wall, facet_domains, 2) # Wall: flux
...
if aneurysm_bc_type == 'neumann':
	...
	bcs_dc = [bc_dc1, bc_dc2, bc_dc3, bc_dc4, bc_dc5, bc_dc6] 
...
file_conc_output = File(results_dir + 'O2_Concentration.pvd')
flux_output = File(results_dir + 'O2_Flux.pvd')
...
while tstep < n_tsteps:
	...
	for tstep_inter in xrange(tsteps_per_file):
		...
		while err > relative_tolerance:
			...
			for bc in bcs_dc:
				bc.apply(K, G)
			solve(K, dc.vector(), G, 'bicgstab', 'default' ) #prec:'bjacobi'
		...
		if tstep % max_save_interval == 0:
			if aneurysm_bc_type == 'dirichlet':
				if MPI.rank(mpi_comm_world()) == 0:
					print 'Calculating the flux ...'

				flux_form = -D * inner(grad(c), n_normal)

				LHS_flux = assemble(flux * phi * ds, keep_diagonal=True)

				RHS_flux = assemble(flux_form * phi * 1e-3 * ds)

				LHS_flux.ident_zeros()

				solve(LHS_flux, flux_sol.vector(), RHS_flux, 'gmres', 'default')

				if MPI.rank(mpi_comm_world()) == 0:
					print 'Calculating the max/min value of flux ...'
